...
# Import model class
from models import HybridModel, quantum_circuit # We still call HybridModel
# Import utilities
from utils import (
    setup_results_directory, write_log, count_parameters,
    plot_training_metrics, evaluate_and_report,
    plot_decision_boundary, plot_quantum_circuit
)

def main():
    # --- 1. SETUP & CONFIGURATION ---
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    experiment_name = f"Moon_MLP_QNN_{timestamp}" # Name for this experiment
    ...
    # Log hyperparameters
    write_log("\n--- Hyperparameters ---")
    write_log(f"Q_CONFIG: {Q_CONFIG}")
    write_log(f"MODEL_CONFIG (Classical Pre-processing): {MODEL_CONFIG}")
    ...
    # We just instantiate HybridModel. It handles its own internal architecture.
    model = HybridModel().to(DEVICE)
    
    print("\n--- Hybrid Model Architecture (MLP -> QLayer) ---")
    print(model)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for features, labels in train_loader:
            ...
            optimizer.step() # Update weights (both classical and quantum)
    ...
    model_save_path = os.path.join(final_result_dir, "hybrid_model_MLP_Q.pth")
