...
from config import (
    ...
    MODEL_CONFIG # This is now the CNN config
)
# Import data loading function
from data_loader import get_mnist_data_loaders
# === CHANGE 1: Import CNN instead of MLP ===
...
def main():
    ...
    # === CHANGE 2: Update experiment name ===
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    experiment_name = f"CNN_MNIST_{timestamp}" # Changed from MLP to CNN
    ...
    # Log hyperparameters
    write_log("\n--- Hyperparameters ---")
    write_log(f"MODEL_CONFIG: {MODEL_CONFIG}") # Will log the CNN config
    ...
    # === CHANGE 3: Instantiate the CNN model ===
    model = CNN().to(DEVICE) # Changed from MLP() to CNN()
    ...
    #   The training loop is identical for MLP and CNN
    #   because both are 'nn.Module' objects.
    #
    ...
    #
    #   !!! NO CHANGES NEEDED HERE !!!
    #   The paths are loaded from config.py, which we already updated.
    #
