...
from config import (
    DEVICE, TRAINING_CONFIG, MODEL_SAVE_PATH,
    MODEL_CONFIG
)
# Import data loading function
from data_loader import get_cifar10_data_loaders
# Import model class (SimpleCNN)
...
def main():
    ...
    experiment_name = f"CNN_CIFAR10_{timestamp}"
    ...
    # Initialize a variable to track the best test loss for Early Stopping
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Early Stopping Logic: Save the model only if test loss has improved
        if test_loss < best_test_loss:
            best_test_loss = test_loss
            best_model_path = os.path.join(final_result_dir, 'best_model.pth')
            torch.save(model.state_dict(), best_model_path)
            print(f"🎉 New best model saved to {best_model_path} with test loss: {test_loss:.4f}")

    print("--- Training Complete ---")
    write_log("--- Training Complete ---")

    # --- 6. SAVE FINAL MODEL AND REPORT RESULTS ---
    
    # Note: We save the *last* model here, but the *best* model was saved during the loop.
    # For evaluation, it's often better to load the 'best_model.pth'.
    final_model_save_path = os.path.join(final_result_dir, 'last_model.pth')
    torch.save(model.state_dict(), final_model_save_path)
    print(f"Final model weights saved to {final_model_save_path}")
    write_log(f"Final model weights saved to {final_model_save_path}")
    ...
    if os.path.exists(best_model_path):
        ...
        print("Warning: No best model found. Evaluating with the last model.")

    # Plotting and Reporting using the BEST model
    ...
    evaluate_and_report(all_labels, all_preds) # Note: this uses the last epoch's preds/labels
