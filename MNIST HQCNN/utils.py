...
def setup_results_directory(experiment_name="hqnn_mnist_experiment"): ### THAY ĐỔI TÊN MẶC ĐỊNH
...
def plot_predictions(model, data_loader, device, num_images=10, filename=PREDICTIONS_PLOT_PATH):
    """Displays and saves the model's predictions on a sample of MNIST images."""
    ...
    ### THAY ĐỔI ### (Tên các lớp của MNIST)
    ...
    for idx in range(num_images):
        ...
        ### THAY ĐỔI ### (Xử lý ảnh xám)
        # Bỏ kênh màu (từ 1, 28, 28 -> 28, 28)
        ...
        std = 0.3081
        img = std * img + mean
        img = np.clip(img, 0, 1) # Đảm bảo giá trị pixel trong khoảng [0, 1]

        ax.imshow(img, cmap='gray') # Thêm cmap='gray'
...
def evaluate_and_report(all_labels, all_preds):
    """Computes and saves the Confusion Matrix and Classification Report using MNIST class names."""
